...
// This is required for sound and animation to synch up properly
    ...
    private function timerCallback(Timer:FlxTimer):Void
    {
        ...
        _text.text = "deBlox Games";
        _curPart ++;

        if (_curPart == 5)
        {
            FlxTween.multiVar(_sprite, { alpha: 0 }, 2.4, { ease: FlxEase.quadIn, complete: onComplete } );
            FlxTween.multiVar(_text, { alpha: 0 }, 2.4, { ease: FlxEase.quadIn } );
        }
    }
    ...
    private function onComplete(Tween:FlxTween):Void
    {
        ...
        FlxG.switchState(Type.createInstance(MenuState, []));
    }
}